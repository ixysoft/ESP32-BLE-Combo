...
public:
  ...
  void move(char x, char y, char wheel = 0, char hWheel = 0);;
  void press(uint8_t btn = MOUSE_LEFT);
  void release(uint8_t btn = MOUSE_LEFT);
  void releaseAll();
...
private:
  ...
  std::string         deviceManufacturer;//蓝牙设备制造商
  uint8_t             batteryLevel;      //蓝牙电池电量
